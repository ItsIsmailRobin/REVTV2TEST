...
export default function Player({
  ...
  useEffect(() => {
    ...
    if (Hls.isSupported()) {
      ...
      // ── Quality upgrade: switch to max quality once buffer is healthy ──
      // Track whether we've already upgraded to max quality on this channel.
      let qualityUpgraded = false;

      const hls = new Hls({
        enableWorker:  true,
        // Disable low-latency mode on all platforms — it trims the buffer
        // too aggressively and causes the loading→play→loading loop on any
        // network hiccup (mobile or PC). A deeper buffer is more important
        // than minimal latency for smooth playback.
        ...
        // ── Buffer sizing ─────────────────────────────────────────────
        // Generous buffer on all platforms — fills up quickly and then
        // holds a deep cushion to absorb network jitter without rebuffering.
        // Mobile gets even deeper buffer to handle cellular/WiFi variance.
        backBufferLength:    isMobile ? 15 : 10,    // keep behind currentTime
        maxBufferLength:     isMobile ? 60 : 40,    // target ahead-buffer
        maxMaxBufferLength:  isMobile ? 120 : 60,   // hard cap — allows deep fill before quality upgrade
        maxBufferSize:       isMobile ? 50_000_000 : 40_000_000, // 50 MB / 40 MB
        maxBufferHole:       isMobile ? 1.5 : 0.5,  // tolerate larger gaps before "stalled"

        // ── Quality / ABR ─────────────────────────────────────────────
        // Start at auto level. Once the buffer is fully loaded, we switch
        // to the highest available quality via the FRAG_BUFFERED event below.
        startLevel:          -1,                    // auto-select on first load
        capLevelToPlayerSize: false,                // allow higher quality than screen size
        // High initial BW estimate — starts at better quality immediately
        // instead of beginning at the lowest rendition.
        abrEwmaDefaultEstimate: isMobile ? 2_000_000 : 5_000_000,
        // Conservative ABR — prefer quality over frequent switches.
        // Stay at current level longer; only drop under sustained stall.
        abrBandWidthFactor:   isMobile ? 0.90 : 0.95,
        abrBandWidthUpFactor: isMobile ? 0.80 : 0.85,
        abrEwmaFastLive:  isMobile ? 3.0 : 4.0,
        abrEwmaSlowLive:  isMobile ? 12.0 : 12.0,

        // ── Stall / nudge ─────────────────────────────────────────────
        // Wait longer for buffer to refill before bailing — reduces the
        // "loading flash" on slow connections.
        maxStarvationDelay:  isMobile ? 12 : 6,
        maxLoadingDelay:     isMobile ? 12 : 6,
        nudgeOffset:         isMobile ? 0.08 : 0.12,
        nudgeMaxRetry:       isMobile ? 20 : 12,
        highBufferWatchdogPeriod: isMobile ? 5 : 4,
        ...
        // ── Live sync ─────────────────────────────────────────────────
        // More segments = deeper buffer = smoother playback on all platforms.
        liveSyncDurationCount:       isMobile ? 4 : 3,
        liveMaxLatencyDurationCount: isMobile ? 12 : 8,
        maxFragLookUpTolerance: isMobile ? 0.5 : 0.25,
      ...
      // ── Max quality upgrade when buffer is healthy ──────────────────
      // Once the buffer is fully loaded (target maxBufferLength reached),
      // switch to the highest available quality level. This gives smooth
      // initial playback at auto quality, then upgrades to max once we
      // have enough buffer cushion to handle the higher bitrate without
      // rebuffering. Works on iOS/Android/PC.
      hls.on(Hls.Events.FRAG_BUFFERED, (_e, data) => {
        if (qualityUpgraded) return;
        const v = videoRef.current;
        if (!v) return;
        // Check how much buffer is ahead of current playback position
        let bufferedAhead = 0;
        ...
        // Switch to max quality once we have at least 20s of buffer ahead
        // (10s on mobile — still safe but upgrades sooner on slow connections)
        const upgradeThreshold = isMobile ? 10 : 20;
        if (bufferedAhead >= upgradeThreshold && hls.levels && hls.levels.length > 1) {
          qualityUpgraded = true;
          // Find the highest quality level (by bitrate)
          const maxLevel = hls.levels.reduce((best, lvl, idx) =>
            lvl.bitrate > (hls.levels[best]?.bitrate ?? 0) ? idx : best, 0
          );
          hls.currentLevel = maxLevel; // lock to max quality
        }
      });
